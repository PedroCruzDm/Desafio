...
import "../Equipes/Equipes.css";
import EquipeModalAdd_adaptar from "../Modals/EquipeModalAdd_adaptar";
import EquipeModalEdit_adaptar from "../Modals/EquipeModalEdit_adaptar";
import EquipeModalDelete from "../Modals/EquipeModalDelete";
import EquipeModalView from "../Modals/EquipeModalView";

const Equipes_adaptar = () => {
  ...
  const fetchEquipes = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:5000/api/equipes_json");
  ...
  const handleSaveNewEquipe = async (equipeData) => {
    try {
      await axios.post("http://localhost:5000/api/equipes_json", equipeData);
      fetchEquipes();
  ...
  const handleUpdateEquipe = async (equipeData) => {
    try {
      await axios.put(
        `http://localhost:5000/api/equipes_json/${selectedEquipe.id}`,
        equipeData
      );
      fetchEquipes();
  ...
  const handleDeleteEquipeConfirm = async () => {
    try {
      await axios.delete(
        `http://localhost:5000/api/equipes_json/${selectedEquipe.id}`
      );
  ...
  return (
    <div className="equipes-container">
      <div className="equipes-header">
        <h2>Gerenciamento de Equipes (JSON)</h2>
      ...
      {showDeleteModal && selectedEquipe && (
        <EquipeModalDelete
          ...
          onConfirm={handleDeleteEquipeConfirm}
        />
      )}
...
export default Equipes_adaptar;